...
function DrawioEditor(id, filename, type, interactive, updateHeight, updateWidth, updateMaxWidth, baseUrl, label) {
	...
	var uiAttr = (
		...
		? "&ui=dark" : "";
	this.iframe = $('<iframe>', {
		src: this.baseUrl + '/?embed=1&proto=json&spin=1&analytics=0&libraries=1&plugins=1&picker=0' + uiAttr + '&lang=' + this.language,
	id: 'drawio-iframe-' + id,
...
function drawioHandleMessage(e) {
	...
	if (!e.origin || !window.drawioEditorBaseUrl 
		|| new URL(e.origin).host != new URL(window.drawioEditorBaseUrl).host)
		return;
